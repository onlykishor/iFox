...
IF NOT DBUSED('eschool')
	OPEN DATABASE data\eschool SHARED
	llOpenDB = True
ENDIF
...
IF EMPTY(cValue) AND NOT EMPTY(cDefault)
	cValue = cDefault
ENDIF
...
LOCAL llRetVal, llBackup
	...
	*=== 3-Mar-2011 run script file instead of executing commands from table
	...
	IF EMPTY(DBC())
		OPEN DATABASE data\eschool SHARED NOUPDATE
	ENDIF
...
*!*********************************************************
* Description
* Insert a record into outbox with WAITING status
FUNCTION SendSMS
...
*!
*!*********************************************************
* Returns available report name for given type
FUNCTION GetSysReport
	PARAMETERS pcType
	...
	IF USED(lcTempCursor)
		SELECT (lcTempCursor)
		IF RECCOUNT(lcTempCursor) > 0
			IF NOT EMPTY(reportfile)
